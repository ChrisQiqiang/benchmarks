...
class BenchmarkCNN(object):
  ...
  def benchmark_with_session(self, sess, supervisor, graph_info,
    ...
    while not done_fn():
      ...
      (summary_str, last_average_loss) = benchmark_one_step(
          ...
          should_output_files=(self.params.variable_update != 'horovod' or is_chief),
          global_step_watcher=global_step_watcher)
  ...
  def _preprocess_graph(self, graph, graph_info):
    ...
    variables_to_keep.update({graph_info.global_images : tf.GraphKeys.GLOBAL_VARIABLES})
  ...
  def _build_model(self):
    ...
    log_rita('Generating {} model'.format(mode_string))
    losses = []
    device_grads = []
    all_logits = []
    all_accuracy_ops = {}
    ...
    for device_num in range(len(self.devices)):
      with tf.name_scope('tower_%i' % device_num) as name_scope, (
        ...
        log_rita('build model in scope tower_{} when building model'.format(device_num))
        if device_num == 0:
