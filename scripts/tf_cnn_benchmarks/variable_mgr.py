...
class VariableMgrLocalFetchFromPS(VariableMgr):
  ...
  def get_gradients_to_apply(self, device_num, gradient_state):
    assert device_num == 0
    #("In PS strategy get_gradients_to_apply, device grads shape is {}".format(gradient_state.get_shape().as_list()))
    device_grads = gradient_state
...
class VariableMgrDistributedFetchFromPS(VariableMgr):
  ...
  def get_gradients_to_apply_on_replicated(self, device_num, gradient_state):
